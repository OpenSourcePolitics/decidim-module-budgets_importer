...
module Decidim::BudgetsImporter
  describe Admin::ProjectForm do
    ...
    describe "when the scope does not exist" do
      ...
      it "assign component scope" do
        expect(subject.scope).to eq(current_component.scope)
      end
    end
    ...
    describe "scope" do
      ...
      context "when the participatory space has a scope" do
        ...
        context "when the scope is not descendant from participatory space scope" do
          ...
          it "find scope in current_component available scopes" do
            expect(form.scope).to be_valid
          end
        end
      end
    end
